...
public class RedisTemplate extends StringRedisTemplate {
    ...
    public static final Map USER_MAP = new HashMap<String, String>();
    private static String redisName = "productCode";
...
//        判断用户是否已经存入redis,如果有则删除信息
        String oldToken = get(redisName + ":user:uid:" + user.getUid());
        if (oldToken != null) {
            super.delete("productCode:user:token:" + oldToken);
        }
...
//        判断用户是否已经存入redis,如果有则删除信息
        String oldToken = get(redisName + "user:uid:" + admin.getId());
        if (oldToken != null) {
